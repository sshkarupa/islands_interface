defmodule IslandsInterfaceWeb.GameChannel do
  ...
  def handle_in("guess_coordinate", params, socket) do
    ...
    case Game.guess_coordinate(via(socket.topic), player, row, col)  do
      {:hit, island, win} ->
        ...
        payload = %{player: player, row: row, col: col, result: result}
        broadcast!(socket, "player_guessed_coordinate", payload)
        {:noreply, socket}
      {:miss, island, win} ->
        ...
        {:noreply, socket}
      :error ->
        {:reply, {:error, %{player: player, row: row, col: col, reason: "Not your turn."}}, socket}
      {:error, reason} ->
        {:reply, {:error, %{player: player, row: row, col: col, reason: reason}}, socket}
    end
  end
  ...
  defp authorized?(socket, screen_name) do
    number_of_players(socket) < 2 && !existing_payer?(socket, screen_name)
  end
  ...
  defp existing_payer?(socket, screen_name) do
    socket
    |> Presence.list()
