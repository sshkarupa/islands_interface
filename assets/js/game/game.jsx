...
function Box(props) {
  return <td className="box">{props.value}</td>;
}

function MessageBox(props) {
  return <div className="message_box">{props.message}</div>;
}
...
class OwnBoard extends React.Component {
  ...
  extractCoordinates(board) {
    let coords = this.state.islands.reduce(function(acc, island) {
  ...
  removeIslandImages() {
    ...
    this.state.islands.forEach(function(island) {
      images[island].remove();
    });
  }
  ...
  processOpponentGuess(response) {
    ...
    if (response.player !== this.state.player) {
      if (response.result.win === "win") {
        ...
        this.setState({
          message:
            "Your opponent forested your " + response.result.island + " island."
        });
        board = hit(board, response.row, response.col);
  ...
  processGuess(response) {
    ...
    if (response.player === this.state.player) {
      if (response.result.win === "win") {
        ...
        this.setState({
          message:
            "You forested your opponent's " +
            response.result.island +
            " island!"
        });
        board = hit(board, response.row, response.col);
      } else if (response.result.hit === true) {
...
class Game extends React.Component {
  ...
  renderStartButtons(props) {
    return (
      <div>
        <Button
          value="Start the Demo Game"
          onClick={() => this.handleClick("start-game")}
        />
        <Button
          value="Join the Demo Game"
          onClick={() => this.handleClick("join-game")}
        />
      </div>
    );
  }
...
if (document.getElementById("islands")) {
  ReactDOM.render(<Game />, document.getElementById("islands"));
}
